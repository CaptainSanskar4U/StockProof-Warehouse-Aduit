      ...
      {(Object.keys(GRAIN_BULK_DENSITIES) as GrainType[]).slice(0, 6).map((g) => (
        <button
          ...
          className={`touch-target px-3 py-1.5 rounded-full border text-xs cursor-pointer ${
            value === g ? 'bg-[#2A2118] text-white border-[#2A2118]' : 'border-[rgba(42,33,24,0.16)] text-[#6B5F4F]'
          }`}
        >
      ...
      {SEASON_ORDER.map((s) => (
        <button
          ...
          className={`touch-target px-3 py-2 rounded-[10px] border text-left cursor-pointer ${
            value === s ? 'bg-[#EFE7D4] border-[#A87F2A]/60' : 'border-[rgba(42,33,24,0.12)]'
          }`}
        >
          <span className="block text-xs text-[#2A2118]">{SEASON_PROFILES[s].name}</span>
        </button>
      ))}
    </div>
  );
...
function SharedPileNote() {
  ...
  return (
    <div className="washi-sheet px-5 py-4 flex items-center gap-4">
      {photo ? (
        <SafeImage src={photo.dataUrl} alt="Shared pile" className="w-14 h-14 rounded-[10px] object-cover border border-[rgba(42,33,24,0.14)] shrink-0" />
      ) : (
        <div className="w-14 h-14 rounded-[10px] border border-dashed border-[rgba(42,33,24,0.25)] shrink-0 flex items-center justify-center">
          <span className="font-mono text-[10px] text-[#8A7D68]">no photo</span>
        </div>
      )}
      <div className="min-w-0">
        <p className="eyebrow-quiet">Same pile as New Audit</p>
        <p className="text-sm text-[#2A2118] truncate">
    ...
    <div className="space-y-5 max-w-3xl mx-auto">
      ...
      <div className="washi-sheet px-5 py-5 space-y-4">
        ...
        <label className="block">
          <span className="text-sm text-[#6B5F4F]">Declared claim (tonnes) — same as New Audit</span>
          <input type="number" value={p.declaredText} onChange={(e) => { p.setDeclaredText(e.target.value); p.setDeclaredTouched(true); }}
            className="mt-1 w-full bg-transparent serif-reading text-4xl text-[#2A2118] focus:outline-none border-b border-[rgba(42,33,24,0.2)] pb-1" />
        </label>
        <div className="grid grid-cols-2 gap-4">
          <label className="block"><span className="text-xs text-[#6B5F4F]">Base diameter {p.baseDiameterMeters.toFixed(1)}m</span>
          ...
          <label className="block"><span className="text-xs text-[#6B5F4F]">Measured height {p.heightMeters.toFixed(1)}m</span>
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      ...
      <div className="washi-sheet px-5 py-5 space-y-4">
        ...
        <label className="block"><span className="text-xs text-[#6B5F4F]">Height {p.heightMeters.toFixed(1)}m</span>
          <input type="range" min={1.5} max={8} step={0.1} value={p.heightMeters} onChange={(e) => p.setHeightMeters(parseFloat(e.target.value))} className="washi-range" /></label>
        <label className="block"><span className="text-xs text-[#6B5F4F]">Diameter {p.baseDiameterMeters.toFixed(1)}m</span>
          <input type="range" min={5} max={22} step={0.1} value={p.baseDiameterMeters} onChange={(e) => p.setBaseDiameterMeters(parseFloat(e.target.value))} className="washi-range" /></label>
        <p className="font-mono text-[11px] text-[#8A7D68]">volume ≈ {coneVolumeM3(p.heightMeters, p.baseDiameterMeters)} m³ · angle atan(2h/d)</p>
      </div>
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      ...
      <div className="washi-sheet px-5 py-5 space-y-4">
        ...
        <GrainRow value={p.grainType} onChange={p.setGrainType} />
        <SeasonRow value={p.season} onChange={p.setSeason} />
        <label className="block"><span className="text-xs text-[#6B5F4F]">Price ₹/tonne (optional)</span>
          <input type="number" value={price} onChange={(e) => setPrice(parseFloat(e.target.value) || 0)}
            className="mt-1 w-full bg-transparent font-mono text-lg text-[#2A2118] border-b border-[rgba(42,33,24,0.2)] focus:outline-none" /></label>
      </div>
