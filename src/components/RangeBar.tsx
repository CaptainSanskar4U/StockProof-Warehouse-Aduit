...
export const RangeBar: React.FC<RangeBarProps> = ({
  ...
  return (
    <div className="w-full select-none">
      {showLabels && (
        <div className="flex items-center justify-between text-xs font-mono text-[#2B2016]/55 mb-1.5">
      ...
      {/* Bar container */}
      <div className="relative h-6 bg-stone-100 rounded border border-[#3D3226]/10 overflow-visible my-3">
        ...
        <div className="absolute inset-0 flex justify-between px-2 items-center pointer-events-none opacity-40">
          <div className="w-px h-2 bg-[#3D3226]/30" />
          <div className="w-px h-2 bg-[#3D3226]/30" />
          <div className="w-px h-2 bg-[#3D3226]/30" />
          <div className="w-px h-2 bg-[#3D3226]/30" />
          <div className="w-px h-2 bg-[#3D3226]/30" />
        ...
        >
          ...
          <div
            className={`w-3.5 h-3.5 rotate-45 rounded-xs shadow-md border-2 border-white ${
              isConsistent
                ? 'bg-emerald-600'
                : isOverDeclared
                ? 'bg-[#B5574F] ring-2 ring-[#B5574F]/30'
                : 'bg-[#B98A2E]'
            }`}
          />
          ...
          <div
            className={`w-0.5 flex-1 ${
              isConsistent ? 'bg-emerald-600' : isOverDeclared ? 'bg-[#B5574F]' : 'bg-[#B98A2E]'
            }`}
          />
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="w-3 h-2 rounded-xs bg-[#B98A2E]/30 border border-[#B98A2E]/60 inline-block" />
          <span className="text-[#2B2016]/55 font-mono">Estimated Bound ({rangeLow.toFixed(1)}–{rangeHigh.toFixed(1)} {unit})</span>
        </div>
        <div className="flex items-center gap-2">
          <span
          ...
          />
          <span className="font-mono text-[#3D3226]">
            Declared Receipt: <strong className={isConsistent ? 'text-emerald-700' : isOverDeclared ? 'text-[#B23A32]' : 'text-[#B98A2E]'}>{declared.toFixed(1)} {unit}</strong>
          </span>
        </div>
      </div>
    </div>
