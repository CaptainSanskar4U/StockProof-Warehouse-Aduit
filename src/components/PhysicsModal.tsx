...
export const PhysicsModal: React.FC<PhysicsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2B2016]/60 backdrop-blur-sm">
      <motion.div
        initial={{ opacity: 0, y: 16, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
        className="bg-white border border-[#3D3226]/10 rounded-2xl max-w-2xl w-full max-h-[90dvh] flex flex-col shadow-[0_24px_64px_rgba(43,32,22,0.25)] overflow-hidden"
      >
        {/* Modal Header */}
        <div className="p-4 sm:p-5 border-b border-[#3D3226]/10 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <Scale className="w-5 h-5 text-[#B98A2E]" />
            <div>
              <span className="text-[10px] font-mono tracking-widest text-[#2B2016]/55 uppercase block">
                MATHEMATICAL FOUNDATION
              </span>
              <h3 className="text-lg font-instrument-serif text-[#3D3226]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 text-[#2B2016]/55 hover:text-[#3D3226] rounded hover:bg-[#3D3226]/5 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Modal Content */}
        <div className="p-5 overflow-y-auto space-y-6 text-sm text-[#2B2016]/70">
          {/* Core Principle Callout */}
          <div className="bg-[#FAF5EB] border border-[#B98A2E]/30 p-4 rounded-xl text-xs space-y-1.5">
            <div className="flex items-center gap-2 text-[#B98A2E] font-mono uppercase tracking-wider font-semibold">
              <ShieldCheck className="w-4 h-4" />
              Core Principle & Golden Rule
            </div>
            <p className="text-[#3D3226] font-medium text-sm">
              "The system supports the auditor — it does not replace the physical audit."
            </p>
            <p className="text-[#2B2016]/55">
          ...
          <div>
            <div className="flex items-center gap-2 text-xs font-mono tracking-wider text-[#B98A2E] uppercase mb-2">
              <Layers className="w-4 h-4" />
              1. Physical Pile Geometry (m³)
            </div>
            <p className="text-xs text-[#2B2016]/55 mb-3">
              Bulk grain poured in floor warehouses forms conical piles determined by the crop's natural angle of repose (~28° to 34°).
            </p>
            <div className="bg-[#F5F0E8] p-3 rounded border border-[#3D3226]/10 font-mono text-xs text-[#3D3226] flex flex-col gap-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs font-mono">
              {Object.entries(GRAIN_BULK_DENSITIES).map(([key, data]) => (
                <div key={key} className="bg-[#F5F0E8] p-2.5 rounded border border-[#3D3226]/10">
                  <div className="text-[#2B2016]/55">{data.name}</div>
                  <div className="text-[#3D3226] font-bold text-sm">{data.density.toFixed(3)} t/m³</div>
                  <div className="text-[10px] text-[#2B2016]/55">Base Moisture: {data.safeMoistureMax}%</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 text-xs">
              <div className="p-3 bg-[#F5F0E8] rounded border border-[#3D3226]/10">
                <strong className="text-[#3D3226] block mb-1">Moisture / Humidity Curve:</strong>
                <p className="text-[#2B2016]/55">
              ...
              <div className="p-3 bg-[#F5F0E8] rounded border border-[#3D3226]/10">
                <strong className="text-[#3D3226] block mb-1">Compaction Multiplier:</strong>
                <p className="text-[#2B2016]/55">
              ...
              <div className="p-3 bg-[#F5F0E8] rounded border border-[#3D3226]/10">
                <strong className="text-[#3D3226] block mb-1">Storage Duration Creep (season-aware):</strong>
                <p className="text-[#2B2016]/55">
              ...
              <div className="p-3 bg-[#2B2016] rounded border border-[#2B2016]">
                <strong className="text-[#D9A441] block mb-2 font-mono text-[11px] uppercase tracking-wider">Season calibration — same fill, different tonnage:</strong>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {SEASON_ORDER.map((s) => {
                    const p = SEASON_PROFILES[s];
                    return (
                      <div key={s} className="bg-white/5 border border-white/15 rounded p-2.5">
                        <div className="text-white text-xs font-mono font-bold uppercase">{p.name}</div>
                        <div className="text-white/50 text-[10px] font-mono mt-0.5">{p.harvestWindow}</div>
                        <div className="text-white/70 text-[11px] mt-1.5 leading-snug">{p.storageHint}</div>
                        <div className="text-[#D9A441] text-[10px] font-mono mt-1.5">
        ...
        <div className="p-4 border-t border-[#3D3226]/10 bg-[#F5F0E8] flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[#2B2016] hover:bg-[#3D3226] text-white font-bold rounded-full text-xs tracking-wider uppercase transition-colors"
          >
            Understood
          </button>
        </div>
